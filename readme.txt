...
= 1.2.3 =
* [変更内容] テンプレート内の class/style 属性出力を `esc_attr()` でエスケープ
* [変更内容] disabled 属性を WordPress の `disabled()` ヘルパーに置き換え
* [変更ファイル] `templates/wizard/wizard-page.php` - class 属性エスケープ
* [変更ファイル] `templates/wizard/steps/step-1-basics.php` - style 属性エスケープ
* [変更ファイル] `templates/wizard/steps/step-4-hours.php` - disabled() ヘルパー使用
* [変更ファイル] `templates/wizard/steps/welcome.php` - class 属性エスケープ
* [変更ファイル] `templates/admin-settings.php` - class 属性エスケープ（4箇所）
* [変更ファイル] `aivec-ai-search-schema.php` - バージョン 1.2.3
* [変更ファイル] `package.json` - バージョン 1.2.3
* [変更ファイル] `readme.txt` - Stable tag 1.2.3
* [互換性] WordPress 6.0+
* [互換性] PHP 8.0+

= 1.2.2 =
* [変更内容] ZipCloud に API endpoint、Terms、Privacy を追加
* [変更内容] Changelog を 1.2.x / 1.0.0 のみに圧縮（開発ログを削除）
* [変更内容] `templates/admin-settings.php`: `$settings_notices` の echo を `wp_kses_post()` でラップ
* [変更内容] `includes/functions.php`: tooltip の echo を `wp_kses_post()` でラップ
* [変更内容] PHPCS ignore コメントを 2 件削減
* [変更ファイル] `readme.txt` - ZipCloud API endpoint 追加、changelog 圧縮
* [変更ファイル] `docs/readme.txt.tpl` - ZipCloud API endpoint 追加
* [変更ファイル] `templates/admin-settings.php` - PHPCS ignore 削除
* [変更ファイル] `includes/functions.php` - PHPCS ignore 削除
* [変更ファイル] `aivec-ai-search-schema.php` - バージョン 1.2.2
* [変更ファイル] `package.json` - バージョン 1.2.2
* [互換性] WordPress 6.0+
* [互換性] PHP 8.0+
* [検証済み] PHPUnit テスト: 全通過
* [検証済み] Lint: エラー 0

= 1.2.1 =
* [変更内容] External Services に API エンドポイントを追加
* [変更内容] Google Maps Geocoding API: `https://maps.googleapis.com/maps/api/geocode/json`
* [変更内容] OpenStreetMap Nominatim: `https://nominatim.openstreetmap.org/search`
* [変更内容] Changelog から禁止ワード文字列を削除（一般的な表現に置換）
* [変更内容] PHPCS 警告を修正（`class-avc-ais-faq-extractor.php` の等号アライメント）
* [変更ファイル] `readme.txt` - External Services に API endpoint 追加、changelog 修正
* [変更ファイル] `includes/class-avc-ais-faq-extractor.php` - PHPCS 警告修正
* [変更ファイル] `aivec-ai-search-schema.php` - バージョン 1.2.1
* [変更ファイル] `package.json` - バージョン 1.2.1
* [互換性] WordPress 6.0+
* [互換性] PHP 8.0+
* [互換性] Pro 版 v1.5.3+ と互換
* [検証済み] WP.org 禁止コードの参照が 0 件
* [検証済み] PHPUnit テスト: 232 件全通過
* [検証済み] Lint: エラー 0、警告 0

= 1.2.0 =
* [変更内容] `vendor/` ディレクトリを配布ZIPから完全除外（PSR-4 フォールバックオートローダー使用）
* [変更内容] `src/License/` ディレクトリを削除（Pro版専用の独自アップデートクライアント）
* [変更内容] スクリプトハンドルをプレフィックス化: `ais-settings` → `avc-ais-settings`
* [変更内容] スクリプトハンドルをプレフィックス化: `ais-wizard` → `avc-ais-wizard`
* [変更内容] ウィザードページスラッグを変更: `ais-wizard` → `avc-ais-wizard`
* [変更内容] `.distignore` を更新: `/vendor`, `/src/License` を除外対象に追加
* [変更内容] `gulpfile.js` を更新: `composer.json`, `package.json`, `package-lock.json` をコピー対象から除外
* [変更内容] GitHub Actions ワークフローを更新: rsync 除外設定を WP.org 準拠に変更
* [変更内容] Pro版専用の独自アップデート機能
* [変更内容] Pro版専用のライセンス検証クライアント
* [変更ファイル] `aivec-ai-search-schema.php` - バージョン 1.2.0
* [変更ファイル] `package.json` - バージョン 1.2.0
* [変更ファイル] `readme.txt` - Stable tag 1.2.0, changelog 追加
* [変更ファイル] `.distignore` - vendor/, src/License/ 除外追加
* [変更ファイル] `gulpfile.js` - paths.copy から不要ファイル削除
* [変更ファイル] `includes/class-avc-ais-settings.php` - ハンドル `avc-ais-settings`
* [変更ファイル] `src/Wizard/Wizard.php` - PAGE_SLUG `avc-ais-wizard`, ハンドル `avc-ais-wizard`
* [変更ファイル] `templates/wizard/wizard-page.php` - ハンドル `avc-ais-wizard`
* [変更ファイル] `tests/phpunit/WizardTest.php` - テスト修正
* [変更ファイル] `tests/phpunit/WizardAjaxTest.php` - テスト修正
* [変更ファイル] `.github/workflows/build-release.yml` - rsync 除外設定更新
* [変更ファイル] `.github/workflows/build-dev.yml` - rsync 除外設定更新
* [互換性] WordPress 6.0+
* [互換性] PHP 8.0+
* [互換性] Pro 版 v1.5.3+ と互換（無料版 1.2.0+ を要求）
* [検証済み] WP.org 禁止コードの参照が 0 件であることを確認
* [検証済み] PHPUnit テスト: 232 件全通過
* [検証済み] ZIP サイズ: 302KB（vendor 除外により大幅削減）

= 1.1.3 =
* [変更内容] Pro ライセンス管理 UI/ロジックを WP.org 版から削除
* [変更内容] 外部サービス（Google Maps Geocoding API / OpenStreetMap Nominatim）を readme に明記
* [変更内容] WordPress.org で不要な `load_plugin_textdomain()` 呼び出しを削除
* [変更内容] WordPress.org 用アセット（スクリーンショット/アイコン）をパッケージから除外
* [変更内容] ヘルパー関数プレフィックスを `avc_ais_` に統一
* [変更内容] スラッグに合わせてインストールパスやバンドル名を更新
* [変更内容] readme の短い説明文を追加
* [変更内容] External Services セクションを追加
* [変更ファイル（抜粋）] `aivec-ai-search-schema.php`（バージョン更新）
* [変更ファイル（抜粋）] `includes/functions.php`（ヘルパー関数のプレフィックス変更）
* [変更ファイル（抜粋）] `templates/admin-settings.php` / `assets/js/admin-settings.js`（ライセンスUI削除）
* [変更ファイル（抜粋）] `readme.txt` / `docs/readme.txt.tpl`（Short description + External Services）
* [変更ファイル（抜粋）] `gulpfile.js` / `bundle`（パッケージ名更新）
* [互換性] 既存機能に変更なし（Pro/ライセンス関連は WP.org 版から削除）
* [互換性] 言語ファイルを再生成

= 1.1.2 =
* [変更内容] 全ての `__()`, `esc_html_e()`, `_e()` 等の翻訳関数呼び出しを更新
* [変更内容] プラグインヘッダーの `Text Domain` を更新
* [変更内容] 翻訳ファイル名を `aivec-ai-search-schema-*.po/mo` に変更
* [変更ファイル] `aivec-ai-search-schema.php` (旧 `ai-search-schema.php`) - ファイル名変更、Text Domain更新
* [変更ファイル] `src/Plugin.php` - Text Domain更新
* [変更ファイル] `src/Schema/class-avc-ais-graphbuilder.php` - PRO版フィルターフック追加
* [変更ファイル] `src/Schema/class-avc-ais-output.php` - CSSクラス名変更
* [変更ファイル] `includes/class-avc-ais-settings.php` - メニュースラッグ、Text Domain更新
* [変更ファイル] `languages/aivec-ai-search-schema.pot` (新規)
* [変更ファイル] `languages/aivec-ai-search-schema-ja.po` (新規)
* [変更ファイル] `languages/aivec-ai-search-schema-ja.mo` (新規)
* [変更ファイル] `languages/aivec-ai-search-schema-ja_JP.po` (新規)
* [変更ファイル] `languages/aivec-ai-search-schema-ja_JP.mo` (新規)
* [変更ファイル] `gulpfile.js` - プラグインファイル名参照更新
* [変更ファイル] `phpunit.xml.dist` - プラグインファイル名参照更新
* [変更ファイル] `tests/phpunit/bootstrap.php` - プラグインファイル名参照更新
* [変更ファイル] `tests/phpunit/*.php` - CSSクラス名参照更新
* [互換性] **既存ユーザーへの影響**: プラグインファイル名の変更により、更新後にプラグインの再有効化が必要な場合があります
* [互換性] **PRO版互換性**: PRO版 v1.5.0 以降で新フィルターフックを使用
* [関連] WordPress.org 審査フィードバック対応
* [関連] PRO版マルチロケーション機能準備

= 1.1.1 =
* [Breaking Changes] Renamed legacy class prefix `AI_Search_Schema_*` → `AVC_AIS_*` (no backward-compat).
* [Breaking Changes] Added explicit public entrypoint `avc_ais_init()` for plugin initialization.
* [Build & Maintenance] Renamed class filenames to match the new prefix for PHPCS compliance.
* [Build & Maintenance] Updated translation catalogs and Composer autoload metadata.

= 1.1.0 =
* [Breaking Changes] Constants: `AI_SEARCH_SCHEMA_*` → `AVC_AIS_*`
* [Breaking Changes] Options: `ai_search_schema_*` → `avc_ais_*`
* [Breaking Changes] Transients: `ai_search_schema_*` → `avc_ais_*`
* [Breaking Changes] AJAX actions: `ai_search_schema_*` → `avc_ais_*`
* [Breaking Changes] Nonces: `ai_search_schema_*` → `avc_ais_*`
* [Breaking Changes] Post meta: `_ai_search_schema_meta` → `_avc_ais_meta`
* [Breaking Changes] Filter/Action hooks: `ai_search_schema_*` → `avc_ais_*`
* [Security Improvements] Added `sanitize_text_field()` to nonce verification in metabox save handler
* [Security Improvements] Added ABSPATH protection to main plugin file and admin settings template
* [Security Improvements] Moved inline styles to `wp_add_inline_style()` for proper enqueuing
* [Documentation] Added comprehensive External Services section to readme.txt documenting:
* [Documentation] Google Maps Geocoding API usage and privacy policy
* [Documentation] OpenStreetMap Nominatim API usage and terms of service
* [Documentation] License validation API (Pro version - future)
* [Build Changes] WordPress.org directory assets (icons, screenshots, banners) are now excluded from the plugin zip file
* [Build Changes] These assets should be uploaded separately via SVN after plugin approval
* [Upgrade Notes] Custom code referencing plugin constants, options, or hooks
* [Upgrade Notes] Database entries with old option names

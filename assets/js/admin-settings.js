(function ($) {
  ...
  function initLicense() {
    const $activateButton = $('#ais-activate-license');
    const $deactivateButton = $('#ais-deactivate-license');
    const $input = $('#ais-license-key');
    const $spinner = $input.closest('.ais-license-key-row').find('.spinner');
    const $message = $('#ais-license-message');

    function showMessage(text, isError) {
      $message.text(text).css('color', isError ? '#dc3232' : '#46b450').show();
      if (!isError) {
        setTimeout(function () {
          $message.fadeOut();
        }, 3000);
    ...
    $activateButton.on('click', function () {
      ...
      $activateButton.prop('disabled', true).text(aisSettings.i18nLicenseActivating || 'Activating...');
      $spinner.addClass('is-active');
      $message.hide();
      ...
      $.ajax({
        ...
        error: function () {
          showMessage('Activation request failed.', true);
        },
        complete: function () {
          $activateButton.prop('disabled', false).text(aisSettings.i18nLicenseActivate || 'Activate');
          $spinner.removeClass('is-active');
        }
      });
    });
    ...
    $deactivateButton.on('click', function () {
      ...
      $deactivateButton.prop('disabled', true).text(aisSettings.i18nLicenseDeactivating || 'Deactivating...');
      $spinner.addClass('is-active');
      $message.hide();
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            showMessage(response.data?.message || 'Deactivation failed.', true);
          }
        },
        error: function () {
          showMessage('Deactivation request failed.', true);
        },
        complete: function () {
          $deactivateButton.prop('disabled', false).text(aisSettings.i18nLicenseDeactivate || 'Deactivate');
          $spinner.removeClass('is-active');
        }
      });
    });
